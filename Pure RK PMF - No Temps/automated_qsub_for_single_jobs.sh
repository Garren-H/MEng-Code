...
for ((i=0; i<total_jobs; i++)); do
  qsub -l walltime=$walltime -N "$job_name"_"$i" -e "$job_name"_"$i".err -o "$job_name"_"$i".out -v include_clusters=$include_clusters,add_zeros=$add_zeros,ARD=$ARD,func_groups_string=\'"$func_groups_string"\',chain_id=$i Pure_PMF_no_temps.sh
done

# Usage: ./automated_qsub_for_single_jobs.sh 1 1 0 "Alkane,Primary alcohol" 8
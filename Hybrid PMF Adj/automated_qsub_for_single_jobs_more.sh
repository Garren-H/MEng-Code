...
for ((i=0; i<total_jobs; i++)); do
  qsub -N "$job_name"_"$i" -e "$job_name"_"$i".err -o "$job_name"_"$i".out -v include_clusters=$include_clusters,include_zeros=$include_zeros,ARD=$ARD,func_groups_string="$func_groups_string",chain_id=$i Hybrid_PMF_more.sh
done

# Usage: ./automated_qsub_for_single_jobs_more.sh 0 0 0 "Alkane.Primary alcohol" 6
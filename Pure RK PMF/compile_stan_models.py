...
def generate_stan_model_code(include_clusters: bool, add_zeros: bool, refT: bool, ARD: bool):
    model_code = '''
        functions {
    ...
    if include_clusters and not refT:
        ...
        model_code += '''
                    ...
                    V[t,m] = V_raw[t,m];'''
    model_code += ''' 
                    V[t,m] = diag_pre_multiply(v_ARD, V[t,m]);     
                }
            }
...
for include_clusters in c:
    for add_zeros in z:
        for refT in t:
            for ARD in a:
                model_code = generate_stan_model_code(include_clusters=include_clusters, add_zeros=add_zeros, refT=refT, ARD=ARD)
                stan_file = f'{path}/Pure_PMF_include_clusters_{include_clusters}_zeros_{add_zeros}_refT_{refT}_ARD_{ARD}.stan'

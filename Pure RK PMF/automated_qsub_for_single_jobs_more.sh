...
for ((i=0; i<total_jobs; i++)); do
  qsub -N "$job_name"_"$i" -e "$job_name"_"$i".err -o "$job_name"_"$i".out -v include_clusters=$include_clusters,variance_known=$variance_known,variance_MC_known=$variance_MC_known,func_groups_string=\'"$func_groups_string"\',chain_id=$i Pure_PMF_more.sh
done

# Usage: ./automated_qsub_for_single_jobs_more.sh 0 1 1 "Alkane,Primary alcohol" 5
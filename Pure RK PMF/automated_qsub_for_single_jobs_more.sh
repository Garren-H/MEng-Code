...
for ((i=0; i<total_jobs; i++)); do
  qsub -l walltime=$walltime -N "$job_name"_"$i" -e "$job_name"_"$i".err -o "$job_name"_"$i".out -v include_clusters=$include_clusters,add_zeros=$add_zeros,refT=$refT,ARD=$ARD,func_groups_string=\'"$func_groups_string"\',chain_id=$i Pure_PMF_more.sh
done

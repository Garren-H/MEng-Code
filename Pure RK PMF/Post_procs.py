...
class Post_process:
    def __init__(self, include_clusters: bool, include_zeros: bool, refT: bool, ARD: bool, 
        ...
        self.path = '/home/garren/HPC Files/Pure RK PMF/Subsets' # path to where files are stored
        self.path += f'/{self.functional_groups[0]}'
        for func in self.functional_groups[1:]:
            self.path += f'_{func}'
        self.data_file = f'{self.path}/data.json'                # data file
        self.path += f'/Include_clusters_{self.include_clusters}/Add_zeros_{self.include_zeros}/RefT_{self.refT}/ARD_{self.ARD}/{self.inf_type}'
    ...
    def get_tensors(self):
        ...
        stan_path = '/home/garren/HPC Files/Pure RK PMF/Stan Models'
        stan_file = f'{stan_path}/Pure_PMF_include_clusters_{self.include_clusters}_zeros_{self.include_zeros}_refT_{self.refT}_ARD_{self.ARD}.stan'
        ...
        for rank in self.ranks:
            ...
            if self.include_clusters and self.refT:
            ...
            elif self.include_clusters and not self.refT:
            ...
            elif not self.include_clusters and self.refT:
            ...
            if self.ARD:
                ...
                v_ARD = 100*np.eye(rank)[np.newaxis,np.newaxis,:,:]
